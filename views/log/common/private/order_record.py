...
def process_main_stage(main_stage, line_num_str, line_res, match_id, log_format_list):
    ...
    len_format = len(log_format_list)
    dic_i = {'_id': str(line_res['time_local']) + '-' + line_num_str}
    for key in line_res:
        if line_res[key]:
           dic_i[key] = line_res[key]
    
    if match_id < 1 or len_format <= 1:
       main_stage.insert(0,dic_i)
       return main_stage
        
    dic_i['match_id'] = match_id
    dic_i['valid'] = 0
    if len(main_stage) >= 1:
       if match_id == 1:
          dic_i['valid'] = 1
          if main[0]['valid'] == 1 and main[0]['match_id'] != len_format:
             for i in range(len_format):
                if i >= len(main) or main[i]['match_id'] == len_format:
                   break
                main[i]['valid'] = 0
       elif main[0]['valid'] == 1:
          if main[0]['match_id'] ==  match_id - 1:
             dic_i['valid'] = 1
          else main[0]['valid'] == 1: #递归，把前面几个设置为无效
             for i in range(len_format):
                if i >= len(main) or main[i]['match_id'] == len_format:
                   break
                main[i]['valid'] = 0
          
    elif match_id == 1: #第一条消息判断
       dic_i['valid'] = 1
